...
#
# plt.figure()
# plt.plot(x.squeeze(), true_y.squeeze(), label="true", color="black", linewidth=3)
...
plt.figure()
for _, run in initial_runs.iterrows():
    history = load_metric_history(run.run_id, "loss", client)
    plt.plot(history.index, history["loss"], label=run.run_id, linewidth=0.5)
plt.yscale("log")
